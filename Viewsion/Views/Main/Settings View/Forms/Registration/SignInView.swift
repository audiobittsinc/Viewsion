...
struct SignInView: View {
        ...
        NavigationView {
            ZStack {
                ...
                Form {
                        ...
                        HStack {
                            Button(action: {
                                if self.isUserInformationValid() {
                                    print("Signing In")
                                    authenticator.signIn(user: UserSignIn(email: email, password: password))
                                }
                            }
                                   , label: {
                                Text(authenticator.isAuthenticating ? "Please wait" : "Sign In")
                                    .font(.title)
                    ...
                    .navigationTitle("Sign In to Account")
                    .font(.title3)
                    .toolbar {
                        Button(action: {
    ...
    private var isLoginDisabled: Bool {
        authenticator.isAuthenticating || email.isEmpty || password.isEmpty
    }
